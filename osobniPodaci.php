...
session_start();
include 'header.php';
if($_SESSION['tip']==2||$_SESSION['tip']==1||$_SESSION['tip']==3){
    ...
    if($baza->updateDB($upit)){
        ...
        if ( $_SESSION['tip']==1){
            ...
            header ("Location: popis_korisnika.php",true);
            ob_end_flush();
            exit;
            }  else {*/
                $kraj="<p>Uspjeno ste obavili sljedeće izmjenje:</p>".$izmjena;
                $_SESSION['izmjene']=$kraj;
                //header ("Location: osobniPodaci.php",true);
...
if ( $_SESSION['tip']==1){
    if($red["status"]=="2"){
        $ispis.='<input type="button" value="Deaktiviraj" class="gumb" onclick="<?php deaktiviraj(); ?>">';
    }elseif ($red["status"]=="1"||$red["status"]=="3" ||$red["status"]=="4") {
        $ispis.='<input type="button" value="Aktiviraj" class="gumb" onclick="<?php aktiviraj(); ?>">';
    }
    
}*/
/*    
function antiviraj(){
$upit="UPDATE korisnik SET status=2 WHERE idkorisnik='".$id."'"; 
$baza->updateDB($upit);
} 

function deaktiviraj(){
$upit="UPDATE korisnik SET status=3 WHERE idkorisnik='".$id."'"; 
$baza->updateDB($upit);
}  
*/
...
$smarty->display('predlosci/osobniPodaci.tpl');
